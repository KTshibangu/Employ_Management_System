...
const CheckInButton = ({ todayRecord, onAction }) => {
    ...
    return (
        <div className='absolute bottom-4 right-4 flex flex-col z-1'>
            <button onClick={handleAttendance} className={`w-full max-w-xs flex justify-between items-center gap-8 p-4 rounded-xl cursor-pointer
            bg-linear-to-br text-white ${isCheckedIn ? "from-slate-700 to-slate-900" : "from-indigo-600 to-indigo-700"}`} disabled={loading}>
                {
                    loading ?
