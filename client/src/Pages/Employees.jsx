    ...
    <div className='animate-fade-in'>
      ...
      <div className='flex flex-col sm:flex-row gap-3 mb-6'>
        <div className='relative flex-1'>
          ...
          <input className='w-full pl-10!' onChange={(e) => setSearch(e.target.value)} value={search} type="text" placeholder='Search Employees...' />
        </div>
            ...
            {/* create employee modL */}
            {
              showCreateModal && (
                <div onClick={() => setShowCreateModal(false)} className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto'>
                  ...
                  <div onClick={(e) => e.stopPropagation()} className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl animate-fade-in'>
                    <div className='flex items-center justify-between p-6 pb-0'>
                      ...
                      <button onClick={() => setShowCreateModal(false)} className='p-2 rounded-lg hover:bg-skate-100 transition-colors text-slate-400 hover:text-slate-600'>
                        <X className='w-5 h-5' />
                      </button>
                    </div>
                    <div className='p-6'>
            ...
            {/* Edit employee modal */}
            {
              editEmployee && (
                <div onClick={() => setEditEmployee(null)} className='fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto bg-black/40 backdrop-blur-sm'>
                  <div onClick={(e) => e.stopPropagation()} className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 aniimate-fade-in'>
                    <div className='flex items-center justify-between p-6 pb-0'>
                      <div>
                      ...
                      <button onClick={() => setEditEmployee(null)} className='p-2 rounded-lg hover:bg-skate-100 transition-colors text-slate-400 hover:text-slate-600'>
                        <X className='w-5 h-5' />
                      </button>
                    </div>
                    <div className='p-6'>
